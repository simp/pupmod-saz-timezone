shared_examples 'Gentoo' do
  ...
  describe "when using default class parameters" do
    ...
    it do
      is_expected.to contain_package('sys-libs/timezone-data').with({
        :ensure => 'present',
    ...
    it { is_expected.to contain_file('/etc/timezone').with_content(/^Etc\/UTC$/) }
    it { is_expected.to contain_exec('update_timezone').with_command(/^emerge --config timezone-data$/) }
    it do
    ...
    context 'when timezone => "Europe/Berlin"' do
      ...
      it { is_expected.to contain_file('/etc/timezone').with_content(/^Europe\/Berlin$/) }
