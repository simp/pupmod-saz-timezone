shared_examples 'RedHat' do
  ...
  describe "when using default class parameters" do
    ...
    it { is_expected.to contain_file('/etc/sysconfig/clock').with_content(/^ZONE="Etc\/UTC"$/) }
    it { is_expected.to_not contain_exec('update_timezone') }
    ...
    context 'when timezone => "Europe/Berlin"' do
      ...
      it { is_expected.to contain_file('/etc/sysconfig/clock').with_content(/^ZONE="Europe\/Berlin"$/) }
    ...
    context 'when RHEL 7' do
      ...
      it { is_expected.to_not contain_file('/etc/sysconfig/clock').with_ensure('file') }
    end
